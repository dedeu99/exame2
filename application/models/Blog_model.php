...
//PONHA O SEU CODIGO ABAIXO DESTA LINHA
	...
	public function get_public_and_private_posts()
	{
			...
			$query = $this->db->query("SELECT u.name, m.id, m.user_id, m.content, m.created_at, m.updated_at, m.likes,m.public from users as u inner join (microposts as m) on (u.id=m.user_id) where m.id='".$_SESSION['id']."' OR public='1' OR (public='0' AND u.id IN (
